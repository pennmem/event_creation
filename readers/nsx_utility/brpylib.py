...
from readers.nsx_utility.brMiscFxns  import openfilecheck, brmiscfxns_ver

# Version control set/check
brpylib_ver        = "1.3.1"
brmiscfxns_ver_req = "1.1.0"
...
class NsxFile:
    ...
    def getdata(self, elec_ids='all', start_time_s=0, data_time_s='all', downsample=1):
        ...
        else:                              start_idx = round(start_time_s * datafile_samp_per_sec)
        if data_time_s == DATA_TIME_DEF:   stop_idx  = STOP_OFFSET_MIN
        else:                              stop_idx  = round((start_time_s + data_time_s) * datafile_samp_per_sec)
        ...
        while not hit_stop:
            ...
            # Read header, check to make sure the header is valid (ie Header field = 0), and skip packets with 0 pts
            if self.basic_header['FileSpec'] != '2.1':
            ...
            timestamp_sample = round(output['data_headers'][-1]['Timestamp'] / self.basic_header['Period'])
            ...
            # data packet 2 as if it was one, and start this process over.
            ...
            if len(output['data_headers']) == 1 and (STOP_OFFSET_MIN < stop_idx < timestamp_sample):
                print("\nBecause of pausing, data section requested is before any data")
                print("was saved, which starts at t = {0:.6f} s".format(
            ...
            max_length           = int((DATA_PAGING_SIZE // downsample_data_size) * downsample_data_size)
        ...
        output['samp_per_s']  = datafile_samp_per_sec / downsample
